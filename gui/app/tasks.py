...
#    app.task_queue = rq.Queue('pangia-tasks', connection=app.redis)
    ...
    try:
        ...
        if args['display_all'] == 'y':
            pangia_da = ' -da'

        pangia_cmd = 'python {}pangia.py -i {} -db {}*.fa -st {} -rm minimap2 -t {} {}{}{}{} -kt -asl {} -ams {} -sm {} -ms {} -mr {} -mb {} -ml {} -mc {} -md {} -mrd {} -o {}'.format(
            args['pangia_loc'],
            pangia_input,
            args['pangia_db'],
            args['seq_type'],
        ...
        # Create scaledown
        tmp_merged_folder = '{}/{}_tmp/merged_sam/'.format(args['tmp_dir'], Path(fastq_split[0]).stem)
        depth_cmd = '{}/depth_scale_down.sh {} {}'.format(
            args['pangia_loc'],
            tmp_merged_folder,
            tmp_merged_folder
        )
        results = os.system(depth_cmd)
        ...
        # Loop through all of the scaledropdown files and move them
        for file in glob.glob('{}*.depth.scaledown'.format(tmp_merged_folder)):
            os.rename(file, '{}/{}'.format(results_folder, Path(file).name))
        if args['preprocess'] == 'y':
...
'''
def create_scaledown(pangia_loc, tmp_dir, fastq_name):
    # Create the scale.dropdown files using the SAM file
    # Form the name of the temp folder, this requires "keep temp" -kt in pangia
    tmp_merged_folder = '{}/{}_tmp/merged_sam/'.format(tmp_dir, fastq_name)
    # Use a find command to get all of the .depth files
    depth_cmd = 'find {} -type f -name \'*.depth\' | cut -d"|" -f3 | sort | uniq > {}/unique_taxid.txt'.format(tmp_merged_folder, tmp_merged_folder)
    results = os.system(depth_cmd)

    unique_taxid = '{}/unique_taxid.txt'.format(tmp_merged_folder)
    # Open up the unique taxid file that has one taxid per line
    with open(unique_taxid) as unique:
        # Loop through each taxid in the file
        for taxid in unique.readlines():

            # Find all of the files with this taxid
            depth_files = glob.glob('{}/*|{}|*.depth'.format(tmp_dir, taxid))
            scaledown_cmd = '{}/depth_scale_down.py {} > {}/{}.depth.scaledown'.format(
                pangia_loc,
                depth_files,
                tmp_dir,
                taxid
...
def run_real_time(user_id, args):
    ...
    try:
        ...
        # Infinite Loop through fastqs in this folder
        while True:
            ...
            if len(fastqs) == 0:
                ...
                for fastq in fastqs:
                    ...
                    # Run PanGIA --- Note difference here between static PanGIA: tmp_dir is output in static, and the SAME directory the sam is put into
                    # (fastq_dir) in the real-time case.
                    pangia_cmd = 'python {}pangia.py -st nanopore -da -db {}*.fa -s {}/master.gawk.sam -o {}/master.results -t {} -kt'.format(
                        args['pangia_loc'],
                        args['pangia_db'],
                        args['fastq_dir'],
                        args['fastq_dir'],
                    ...
                    mv_depthfiles = os.system('cp -f {}/master.results/master.gawk_tmp/merged_sam/*.depth {}/'.format(args['fastq_dir'], results_folder))
                    ...
                    """
                    # Loop through all of the scaledropdown files and move them
                    for file in glob.glob('{}*.depth.scaledown'.format(results_folder)):
                        os.rename(file, '{}/{}'.format(results_folder, Path(file).name))
                    """
                    if args['preprocess'] == 'y':
                        os.rename('{}/fastp.html'.format(args['fastq_dir']), '{}/fastp.html'.format(results_folder))
                        os.rename('{}/fastp.json'.format(args['fastq_dir']), '{}/fastp.json'.format(results_folder))
...
# Testing GUI and Redis/RQ communication inside Docker-Compose:
#def example(multiple):
#    print('Starting task. Multiplying by 20 ...')
#    result = multiple*20
#    time.sleep(1)
#    print(result)
